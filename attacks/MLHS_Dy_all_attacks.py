    ...
    def generate_np(self, x_list, **kwargs):
        ...
        for i in range(total):
                ...
                if y[i] is not None:
                    ...
                    print('sample pre:', np.argwhere(pred_bin > 0).flatten(),
                          'sample ground truth:', np.argwhere(y_bin > 0).flatten())
                ...
                if method == 'SA':
                    ...
                    print('q_max', q_max)
                    eps = 0.05
                    x_adv1, count_tem, fs1 = SquareAttack(
                        pop_size, q_max, generation, img_rows * img_cols * nchannels,
                        self.model, x_list[i], target_label, eps, batch_size, gradient=None
                    )
                    queries = count_tem

                elif method == 'MLHS-DyF108':
                    print("=" * 70)
                    # q_max = 1
                    print('q_max', q_max)
                    eps = 0.05
                    ...
                    x_adv2, count_tem2, fs2 = mlae_latent_bicubic_de_full(
                        pop_size, q_max, generation,
                        self.model, x_list[i], target_label,
                        eps, batch_size,
                    ...
                    # 在你的 generate_np 中，MLHS-DY 攻击成功后

                    if fs2 == 1:
                        x_adv1, q_refine, info = refine_adversarial_visual(
                            x_list[i], x_adv1, self.model, target_label, eps,
                            max_queries=q_max - queries,
                            q_limit=q_max,
                            verbose=True
                        )
                        queries += q_refine
                    ...
                    print('q_max', q_max)
                    eps = 0.05
                    x_adv1, count_tem, fs1 = DE(
                        pop_size, generation, img_rows * img_cols * nchannels,
                        self.model, x_list[i], target_label, eps, batch_size, gradient=None
                    )
                    ...
                    x_adv1, count_tem1, fs1 = SimBA_attack(
                        pop_size, q_max, generation, img_rows * img_cols * nchannels,
                        self.model, x_list[i], target_label, eps, batch_size, gradient=None
                    )
                    queries = count_tem1
                ...
                print(f"\n[Sample {i}] {method} | TargetMatch={match_target} | "
                      f"L2={l2:.2f} | LPIPS={lpips_val:.4f} | Queries={queries}")


                ### 保存结果####
                # 全量收集：无论攻击成功与否，均保存当前样本
        ...
        np.save(f"{save_root}{method}_{q_max}_queries.npy", np.array(queries_list))  # ← 新增
        ...
        print(f"  queries: {method}_{q_max}_queries.npy shape={np.array(queries_list).shape}")  # new
        ...
        # 保持接口兼容性（原函数无显式 return，此处返回空结构避免外部报错）
...
class Problem_eps:
    ...
    def evaluate_adv(self, x_adv):
        ...
        with torch.no_grad():
            if torch.cuda.is_available():
                predict = self.model(torch.tensor(np.clip(x_adv, 0.,
...
class BicubicLatentMapper:
    ...
    def __call__(self, z):
        ...
        delta = torch.nn.functional.interpolate(
            z_t, size=(self.h, self.w), mode='bicubic', align_corners=False
        )
        return delta.squeeze(0).numpy()


def mlae_latent_bicubic_de_full(
        pop_size, q_max, generation, model, image, target_label,
        eps, batch_size, block_size=16, gradient=None, hide_labels=None,
        grid_size=3, region_init_strategy='region',
        color_coherence=0.2
):
    ...
    print(
        f"[LatentBicubic] block_size={block_size}, latent_dim=({c},{n_h},{n_w}), "
        f"total={c * n_h * n_w}, color_coh={color_coherence}")
    ...
    if use_region:
        block_mask = np.zeros((n_h, n_w), dtype=bool)
        bh1 = max(0, region[0] // block_size)
        bw1 = max(0, region[2] // block_size)
        bh2 = min(n_h, (region[1] + block_size - 1) // block_size)
        bw2 = min(n_w, (region[3] + block_size - 1) // block_size)
        block_mask[bh1:bh2, bw1:bw2] = True

    # ========== 阶段3：初始化 x_adv（不变） ==========

    if use_region and region_init_strategy == 'region':
        x_adv = x_ret_full.copy()
        for ch in range(c):
            x_adv[ch, ~pixel_mask] = image[ch, ~pixel_mask]
    else:
        x_adv = x_ret_full.copy()
        if use_region:
            for ch in range(c):
    ...
    if current_fitness == 0.0:
        if use_region and verify_adversarial(model, x_adv, target_label):
    ...
    for gen in range(generation):
        ...
        for i in range(pop_size):
            ...
            # mutant = np.clip(mutant, -Z_MAX, Z_MAX)
            if use_region:
                mutant[:, ~block_mask] = 0
            ...
            if color_coherence < 1.0 and c == 3:
                ...
                # z_trial = np.clip(z_trial, -Z_MAX, Z_MAX)
            ...
            ##########评估子代#################
            if use_region:
                delta_trial[:, ~pixel_mask] = 0
            x_adv_trial = np.clip(x_adv + delta_trial, 0.0, 1.0)
            if use_region:
                for ch in range(c):
                    x_adv_trial[ch, ~pixel_mask] = image[ch, ~pixel_mask]
        ...
        if best_fitness < current_fitness:
            ...
            if color_coherence < 1.0 and c == 3:
                ...
                # z_best = np.clip(z_best, -Z_MAX, Z_MAX)
            best_delta = mapper(z_best) * eps
            #############映射到图像#################


            if use_region:
                best_delta[:, ~pixel_mask] = 0
            ...
            if (gen + 1) % 10 == 0:
                print(
                    f"[LatentBicubic] Gen {gen + 1}: best={best_fitness:.4f}, current={current_fitness:.4f}, queries={eval_count} (no update)")
        ...
        for idx in worst_idx:
            ...
            if use_region:
                mask = mask & block_mask[np.newaxis, :, :]
    ...
    print(
        f"[LatentBicubic] Done. fitness={gbest_fitness:.4f}, L2={l2_final:.2f}, queries={eval_count}, success={success}")
...
def SquareAttack(pop_size, q_max, generation, length, model, image, target_label, eps, batch_size, gradient=None):
    ...
    for i_iter in range(n_iters - 1):
        ...
        # 防止尝试无效的 delta（clip 后不变）
        x_window = image[:, center_h:center_h+s, center_w:center_w+s]
        x_best_window = x_best[:, center_h:center_h+s, center_w:center_w+s]
        while np.sum(np.abs(np.clip(x_window + delta[:, center_h:center_h+s, center_w:center_w+s], min_val, max_val) - x_best_window) < 1e-7) == c * s * s:
...
class SimBA:
    ...
    def __init__(self, model, image_size):
        ...
        self.model.eval()
        self.query_count = 0  # 【改动1】新增查询计数器

    # ==================== 内嵌 utils 功能（开始）====================

    @staticmethod
    def diagonal_order(image_size, channels):
        """内嵌自 utils.py：对角线扫描顺序"""
        ...
        for i in range(1, image_size):
            reverse = order[image_size - i - 1].index_select(
                0, torch.LongTensor([j for j in range(i - 1, -1, -1)])
            )
    ...
    # 【保持官方】expand_vector 维持 CHW (batch, 3, H, W) 格式
    ...
    def get_fitness(self, x, target_labels):
        """
        多标签攻击适应度（无 L2 惩罚）。
        target_labels: 需要最终显示为正（>=0.5）的标签索引列表。
        """
        # 【修正】添加 .detach() 避免 requires_grad 报错
    ...
    # 【改动4】simba_single：适配多标签，使用适应度越小越好
    def simba_single(self, x, target_labels, num_iters=10000, epsilon=0.2):
        """
        单样本 SimBA 攻击。
        x: (1, c, h, w) 或 (c, h, w) tensor，值域 [0,1]
        返回: (x_adv, queries, final_fitness)
        ...
        for i in range(num_iters):
            ...
            if i % 200 == 0:
                print(f'SimBA iter {i}: fitness={last_fitness:.6f}, queries={self.query_count}')
...
def SimBA_attack(pop_size, q_max, generation, length, model, image, target_label, eps, batch_size, gradient=None):
    ...
    参数：
        pop_size, generation, length, batch_size, gradient: 保持接口一致性，未实际使用
        q_max: 最大查询次数（作为 num_iters）
        model: 目标模型，接收 [0,1] 范围的 (N,3,H,W) tensor
        image: numpy array (c, h, w) 或 tensor，值域 [0,1]
        target_label: np.array / list / int，需要为正的标签索引（如 [11, 14]）
        eps: 单维度扰动步长

    返回：
        x_adv: numpy array (c, h, w)
        queries: int
        success: int (1/0)
    """
    # 图像预处理
    ...
    # 使用 simba_single 执行攻击（你的场景为单样本）
    x_adv, queries, final_fitness = simba.simba_single(
        x, target_labels, num_iters=q_max, epsilon=eps
    )
...
def evaluate_saved_npy(model, tmp_folder_path, method, q_max):
    """
    读取已保存的 .npy 文件，复现与原代码完全一致的评估结果。
    路径: ../{tmp_folder_path}/{method}/{q_max}/
    """
    save_root = f"../{tmp_folder_path}/{method}/{q_max}/"

    # ---------- 加载 .npy ----------
    ori = np.load(f"{save_root}{method}_{q_max}_ori.npy")
    adv = np.load(f"{save_root}{method}_{q_max}_adv.npy")
    ori_y = np.load(f"{save_root}{method}_{q_max}_ori_y.npy")
    adv_tar = np.load(f"{save_root}{method}_{q_max}_adv_tar.npy")
    queries_arr = np.load(f"{save_root}{method}_{q_max}_queries.npy")

    total = len(ori)
    print(f"[LoadNPY] Loaded {total} samples from {save_root}")
    print(f"  ori shape:      {ori.shape}")
    print(f"  adv shape:      {adv.shape}")
    print(f"  ori_y shape:    {ori_y.shape}")
    print(f"  adv_tar shape:  {adv_tar.shape}")
    print(f"  queries shape:  {queries_arr.shape}\n")
    ...
    for i in range(total):
        ...
        # 1. 验证攻击目标
        with torch.no_grad():
            x_t = torch.tensor(np.expand_dims(x_adv, axis=0), dtype=torch.float32)
            if torch.cuda.is_available():
                x_t = x_t.cuda()
        ...
        pred_bin[pred_bin >= 0.5] = 1
        pred_bin[pred_bin < 0.5] = -1
        match_target = np.all((pred_bin == y_target), axis=1)[0]

        # 2. 计算扰动指标（与原代码完全一致）
        diff = x_adv - x_ori
        diff_255 = ((x_adv / 2 + 0.5) * 255) - ((x_ori / 2 + 0.5) * 255)
        l1 = np.linalg.norm(diff.flatten(), ord=1)
        l2 = np.linalg.norm(diff.flatten(), ord=2)
        linf = np.linalg.norm(diff.flatten(), ord=np.inf)
        meanr = np.mean(np.abs(diff))
        ...
        # 3. 计算 LPIPS（手动 [0,1] -> [-1,1] 归一化）
        orig_tensor = torch.from_numpy(x_ori).float().unsqueeze(0)
        adv_tensor = torch.from_numpy(x_adv).float().unsqueeze(0)
        if torch.cuda.is_available():
            orig_tensor = orig_tensor.cuda()
            adv_tensor = adv_tensor.cuda()
